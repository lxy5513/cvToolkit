'''
PoseTrack 
Record in Coco keypoins format
'''
...
def light_track_camera(pose_estimator, video_capture):
    ...
    while video_capture.isOpened():
        ...
        if img_id == 0:
            ...
            out = video_out(video_capture, cur_img, 'hrnet_result.mp4')
        ...
        if is_keyframe(img_id, keyframe_interval) or flag_mandatory_keyframe:
            ...
            for det_id in range(num_dets):
                ...
                keypoints, coco_kps = inference_keypoints(pose_estimator, bbox_det_dict, cur_img)
        ...
        ## 非关键帧
            ...
            for det_id in range(num_dets):
                ...
                st_time_pose = time.time()
                keypoints_next, coco_kps = inference_keypoints(pose_estimator, bbox_det_dict_next, cur_img)
                end_time_pose = time.time()
                total_time_POSE += (end_time_pose - st_time_pose)
...
def inference_keypoints(pose_model, test_data, raw_img):
...
def visualize_img(img, candidates, img_id, change_color=None):
    ...
    for candidate in candidates:
        ...
        HistoryRecords[track_id].append(candidate["coco_kps"])
        if track_id == 0:
            text_id = '0'
        else:
            text_id = track_id
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video', '-v', type=str, dest='video_path', default="data/videos/football.mp4")
